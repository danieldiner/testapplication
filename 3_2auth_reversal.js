...
function processPost(bodyParam, uri = '', callback) {
    var resource = "/pts/v2/payments/6309594943106236904005/reversals";
    var uri = "";
    var method = "post";
    var statusCode = -1;
...
function standaloneHttpSignature(callback) {
    ...
    processPost(bodyParam, uri = '', function (error, data, response, statusCode) {
        if (statusCode == 0) {
            console.log("\nSTATUS : SUCCESS Second Call (Reversal) (HTTP Status = " + statusCode + ")");
            var data1 = data
            console.log('data of the request', data)

        }
        else {
            console.log("\nSTATUS : ERROR Second Call (Reversal)  (HTTP Status = " + statusCode + ")");
        }
    });
