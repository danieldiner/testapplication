...
function processPost(bodyParam, uri = '', callback) {
    var resource = "/risk/v1/decisions/6309621064796272004005/marking";
...
function standaloneHttpSignature(callback) {
    ...
    processPost(bodyParam, uri = '', function (error, data, response, statusCode) {
        if (statusCode == 0) {
            console.log("\nSTATUS : SUCCESS Decision Manager (HTTP Status = " + statusCode + ")");
        }
        else {
            console.log("\nSTATUS : ERROR Single Sale Auth+Capture (HTTP Status = " + statusCode + ")");
        }
    });
