...
function trimUrl(hrefResponse, hrefToRemove) {
	if (hrefResponse.startsWith(hrefToRemove)) {
		return hrefResponse.slice(hrefToRemove.length)
	} else {
		return str
	}
}
...
function processPost(bodyParam, uri = '', callback) {
	...
	console.log('This is resourceeeeee 1//////', resource)
...
function standaloneHttpSignature(callback) {
	...
	processPost(bodyParam, uri = '', function (error, data, response, statusCode) {
		if (statusCode == 0) {
			console.log("\nSTATUS : SUCCESS First Call (HTTP Status = " + statusCode + ")");
	
				
			
		}
		else {
			console.log("\nSTATUS : ERROR First Call (HTTP Status = " + statusCode + ")");
		}
	});
